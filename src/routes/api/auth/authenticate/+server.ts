...
export const POST: RequestHandler = async ({ request, platform }) => {
	...
	const credential = await db
		.prepare('SELECT id, user_id, public_key, counter, transports FROM passkey_credentials WHERE id = ?')
	...
	const publicKeyBytes = new Uint8Array(credential.public_key.split(',').map(Number));
	...
	try {
		...
		return jsonError(400, 'verification_failed', `Authentication verification failed: ${e instanceof Error ? e.message : 'Unknown error'}`);
	}
