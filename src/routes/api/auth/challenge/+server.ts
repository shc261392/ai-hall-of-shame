...
export const GET: RequestHandler = async ({ request, platform, url }) => {
	...
	if (purpose !== 'registration' && purpose !== 'authentication') {
		return jsonError(400, 'invalid_request', 'Query param "purpose" must be "registration" or "authentication"');
	}
	...
	await db.batch([
		...
		db
			.prepare('INSERT INTO challenges (id, challenge, ip_address, purpose) VALUES (?, ?, ?, ?)')
